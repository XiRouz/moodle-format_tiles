...
// SCUSTOM BEGIN
function format_tiles_get_grades_for_cm($mod) {
    global $USER;

    $gradesinfo = grade_get_grades($mod->get_course()->id, 'mod', $mod->modname, $mod->instance, $USER->id);
    $grades = ['completed' => 0, 'outof' => 0];
    ...
    if (!empty($gradesinfo) && !empty($gradesinfo->items)) {
        foreach ($gradesinfo->items as $item) {
            if ($item->scaleid !== null) { // scaleid is null for grade items of GRADE_TYPE_NONE
                $grades['outof'] = round($item->grademax, 2);
                if (!empty($item->grades)) {
                    $grade = end($item->grades);
                    $grades['completed'] = round($grade->grade, 2);
                } else {
                    $grades['completed'] = 0;
                }
            }
