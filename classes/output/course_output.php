...
class course_output implements \renderable, \templatable {
                ...
                if ($section->visible && $this->completionenabled) {
                    if (isset($this->modinfo->sections[$sectionnum])) {
                        ...
                        $completionthistile = $this->section_progress($this->modinfo->sections[$sectionnum], $this->modinfo->cms,
                            $this->courseformatoptions['courseshowtileprogress']);
     ...
     * @param int $tileprogressmode tile progress view mode
     * @return array with the completion data x items complete out of y
     */
    public function section_progress($sectioncmids, $coursecms, $tileprogressmode = 0) {
        $completed = 0;
        $outof = 0;
        foreach ($sectioncmids as $cmid) {
            ...
            if ($thismod->uservisible && !$thismod->deletioninprogress) {
                if ($this->completioninfo->is_enabled($thismod) != COMPLETION_TRACKING_NONE) {
                    // SCUSTOM BEGIN
                    if ($tileprogressmode == 3) {
                        $grades = format_tiles_get_grades_for_cm($thismod);
                        $outof += $grades['outof'];
     ...
     */
    public function completion_indicator($numcomplete, $numoutof, $aspercent, $isoverall, $tileprogressmode = 0) {
        ...
        $progressdata['isTotalGradeMode'] = $tileprogressmode == 3 ? true : false;
